const buttons = document.querySelectorAll('.btn');
const input_display = document.querySelector('.display .input');
const output_display = document.querySelector('.display .output');

let input = "";

for (let btn of buttons) {
    const value = btn.dataset.btn;

    btn.addEventListener('click' , () => {
        if(value == "clear") {
            input = "";
            input_display.innerHTML = "";
            output_display.innerHTML = "";
        } else if (value == "backspace") {
            input = input.slice(0, -1);
            input_display.innerHTML = inputclean(input);
        } else if (value == "=") {
            let result = eval(percent_debug (input));
            output_display.innerHTML = output_clean (result);
        } else if (value == "()") {
            if (input.indexOf("(") == -1 || 
                input.indexOf("(") != -1 && 
                input.indexOf(")") != -1 && 
                input.lastIndexOf("(") < input.lastIndexOf(")")) {
                input += "(";
            } else if (
                input.indexOf("(") != -1 && 
                input.indexOf(")") == -1 || 
                input.indexOf("(") != -1 &&
                input.indexOf(")") != -1 &&
                input.lastIndexOf("(") > input.lastIndexOf(")")) {
                input += ")";
            }
            input_display.innerHTML = inputclean(input);
        } else {
            if (dot_input (value)) {
                input += value;
            input_display.innerHTML = inputclean(input);
            }
            
        }
    })
}
//? func for operators and their view
function inputclean (input) {
    let input_array = input.split("");
    let input_array_length = input_array.length;
    //loop here!
    for (let i=0; i<input_array_length; i++) {
        if(input_array[i] == "*") {
            input_array[i] = `<span class="operator">x</span>`
        } else if (input_array[i] == "/") {
            input_array[i] = `<span class="operator">:</span>`
        } else if (input_array[i] == "-") {
            input_array[i] = `<span class="operator">-</span>`
        } else if (input_array[i] == "+") {
            input_array[i] = `<span class="operator">+</span>`
        } else if (input_array[i] == "(") {
            input_array[i] = `<span class="brackets">(</span>`
        } else if (input_array[i] == ")") {
            input_array[i] = `<span class="brackets">)</span>`
        } else if (input_array[i] == "%") {
            input_array[i] = `<span class="percent">%</span>`
        }
    }
    return input_array.join("");
}
// ? func for decimal after every third number
function output_clean (output) {
    let output_str = output.toString();
    let decimal = output_str.split(".")[1];
    output_str = output_str.split(".")[0];
    let output_array = output_str.split("");
     //loop here!
    if (output_array.length > 3) {
        for (let i = output_array.length - 3; i>0; i-= 3) {
            output_array.splice(i, 0, ",");
        }
    }
    if(decimal) {
        output_array.push(".");
        output_array.push(decimal);
    }
    return output_array.join("");
}
function dot_input (value) {
    let last_input = input.slice(-1);
    let operators = ["-", "+", "*", "/"]
    if(value == "." && last_input == ".") {
        return false;
    }
    // if (value == "")
    if(operators.includes(value)) {
        if(operators.includes(last_input)) {
            return false;
        } else {
            return true;
        }
    }
    return true;
}
function percent_debug (input) {
    let input_array = input.split("");

    for (let i = 0; i<input_array.length; i++) {
        if (input_array[i] == "%" ) {
            input_array[i] = "/100";
        }
    }
    return input_array.join("");
}
